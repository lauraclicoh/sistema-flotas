...
def _df_to_rows(df: pd.DataFrame) -> list:
    """Convierte DataFrame a lista de listas de strings seguros."""
...
def reemplazar_hoja(nombre_hoja, df: pd.DataFrame):
    """Reemplaza hoja completa. Usa conversión segura celda por celda."""
    try:
        sh = conectar_sheets()
        ...
        ws.clear()
        if not df.empty:
            # Limpiar columnas temporales y duplicadas antes de subir
...
# ================= CACHÉ EN MEMORIA =================
# FIX PROBLEMA 1 y 2: El historial se guarda en session_state y se actualiza
# localmente al guardar cada gestión. Así el aliado desaparece INMEDIATAMENTE
# del pool sin esperar una nueva lectura de Sheets.
...
def _get_base():
    if "base_df" not in st.session_state or st.session_state.get("base_stale", True):
        ...
        if df.empty:
            ...
            # Eliminar columnas duplicadas (conserva la primera ocurrencia)
...
# ================= LÓGICA CRM =================
# FIX PROBLEMA 3: Los aliados no-responden NO salen del pool inmediatamente.
# La función calcular_proxima() decide cuándo volver a llamarlos según las reglas.
# filtrar_pool() aplica esas reglas al construir el pool de cada analista.

def calcular_proxima(resultado, estado, razon, intentos):
    """
    Reglas de recontacto:
    - NO_VOLVER: Rechaza / Empleado / Point / No le interesa → nunca vuelve
    - No contestó → 1 día (máx 10 intentos, luego 30 días)
    - Apagado / Fuera de servicio → 2 días (máx 10 intentos, luego 30 días)
    - Número errado → 3 días
    - Interesado llega a cargue / Fleet no acepta HUB → 5 días
    - Interesado esporádico → 3 días
    """
    hoy    = datetime.now()
    ...
    # 🔁 Sin contacto — con límite de intentos
    if resultado in ["No contestó","Apagado","Fuera de servicio","Número errado"]:
        ...
        if resultado in ["Apagado","Fuera de servicio"]:
...
def filtrar_pool(df):
    """
    Aplica reglas CRM al pool:
    - Elimina NO_VOLVER permanentemente
    - Elimina aliados cuya proxima_gestion sea futura (están en pausa)
    - Los que ya cumplieron su pausa vuelven a aparecer automáticamente
    """
    if "proxima_gestion" not in df.columns:
        return df
    df = df.copy()
    # Quitar bloqueados permanentes
    df = df[df["proxima_gestion"].astype(str).str.upper() != "NO_VOLVER"]
    # Quitar los que tienen recontacto futuro
    def disponible(v):
        v = str(v).strip()
        if v in ("","nan","None","0"):
            return True   # sin gestión previa → disponible
...
def guardar_gestion(row):
    ...
    st.session_state["hist_stale"] = False  # ya actualizado localmente

def actualizar_base_crm(identificacion, resultado, estado, razon):
    """Actualiza la fila del aliado en BASE con CRM. Lee Sheets solo una vez."""
    df = leer_hoja("BASE")
    ...
    df.loc[idx,"ultimo_resultado"] = _safe_str(resultado)
    df.loc[idx,"ultimo_estado"]    = _safe_str(estado)
    df.loc[idx,"ultima_razon"]     = _safe_str(razon)
    df.loc[idx,"fecha_gestion"]    = _safe_str(datetime.now())
    df.loc[idx,"intentos"]         = intentos_n
    df.loc[idx,"proxima_gestion"]  = _safe_str(proxima)
    reemplazar_hoja("BASE", df)
    _invalidar_base()

def procesar_incremental(df_nuevo):
    """
    Cruce incremental robusto:
    - IDs nuevos → se agregan con CRM en blanco
    - IDs existentes → actualizan datos operativos CONSERVANDO historial CRM
    FIX: usa _df_to_rows para conversión segura, evitando el error de dtype.
    """
    ...
    # Limpiar NaN en todo el df_nuevo
    df_nuevo = df_nuevo.fillna("")

    if base_actual.empty:
        # Primera carga: agregar columnas CRM en blanco
        for col in COLS_CRM:
    ...
    # Nuevos: no están en la base actual
    nuevos = df_nuevo[~df_nuevo["identificacion"].isin(ids_viejos)].copy()
    for col in COLS_CRM:
        nuevos[col] = "0" if col == "intentos" else ""

    # Existentes: actualizar datos operativos, conservar CRM
    # Solo actualizamos las columnas que NO son CRM
    ...
    # Actualizar solo columnas operativas en los existentes
        ...
        if hist.empty:
            ...
            if d.empty:
                ...
                emb=pd.DataFrame({"Etapa":["Llamados","Contactados","Interesados"],
                                  "Cantidad":[tot,g,it],
                                  "%":[100,round(g/tot*100,1) if tot else 0,
                                       round(it/tot*100,1) if tot else 0]})
                ...
                st.plotly_chart(px.line(tend,x="fecha",y="llamadas",
                                        title="Tendencia diaria",markers=True),use_container_width=True)
...
- Solo cuando tengas aliados nuevos o datos actualizados.
- La base permanece en Google Sheets **indefinidamente** — no desaparece.
- Usa **Incremental** siempre que puedas para conservar el historial CRM.
        """)
        ...
        if archivo:
            try:
                ...
                if "Incremental" in modo:
                    st.markdown("""
**Lo que hace el cruce incremental:**
- 🆕 IDs nuevos → se añaden con intentos=0 y CRM en blanco
- ♻️ IDs existentes → actualizan teléfono, zona, vehículo, etc. **conservando** intentos, último resultado y próxima gestión
                    """)
                    ...
                    st.warning("⚠️ Esto borrará TODA la base actual incluyendo el historial CRM de cada aliado.")
...
| No le interesa | ❌ Bloqueo permanente | Nunca |
        """)
        st.info("Estas reglas se aplican automáticamente. Los aliados en pausa vuelven a aparecer solos cuando se cumple el tiempo.")
...
if perfil == "Analista":
    ...
    with tab_g:
        ...
        pool=filtrar_pool(pool)  # aplica reglas de pausa y bloqueo

        if pool.empty:
            st.info("No hay aliados disponibles para este filtro. "
                    "Los aliados en pausa volverán cuando se cumpla su tiempo de recontacto.")
            st.stop()
        ...
        # FIX PROBLEMA 1 y 2: Quitar ya gestionados HOY usando caché local
        # El caché se actualiza inmediatamente al guardar, sin esperar Sheets
        if not hist.empty:
            hv=hist.dropna(subset=["fecha"])
        ...
        # Pool activo — basado en REPARTO + caché local hist
        hoy_s=datetime.now().date().isoformat()
        ...
        # FIX PROBLEMA 1: Quitar gestionados usando caché local (sin llamar Sheets)
        ...
        if mis_ids:
            ...
            if sub:
                ...
                if res=="Sí contestó" and er is None:
                    ...
                    st.success(f"✅ Guardado. Próximo recontacto calculado automáticamente.")
                    st.rerun()
        else:
        ...
        if h_local2.empty:
            ...
            if mhist.empty:
                ...
                if mf.empty:
                    ...
                    st.plotly_chart(px.bar(td,x="fecha",y="llamadas",
                                          title="Mis llamadas por día"),use_container_width=True)
                    st.markdown("---")
