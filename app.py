...
def reemplazar_hoja(nombre_hoja, df: pd.DataFrame):
    try:
        ...
        if not df.empty:
            ...
            data = [headers] + _df_to_rows(df_clean)
            ws.update(data)
    except Exception as e:
...
def _norm_vh(v):
    ...
    if "moto" in v:
        return "Moto"
    if any(k in v for k in ["camion", "camión", "truck", "npr"]):
        return "Camión"
    return str(v).title()
    ...
    if "base_df" not in st.session_state or st.session_state.get("base_stale", True):
        ...
        if df.empty:
            ...
            if col_f is not None:
                # Tomar solo la primera columna si hay duplicados
                serie = df[col_f]
                if isinstance(serie, pd.DataFrame):
                    serie = serie.iloc[:, 0]
                # Convertir a string plano para evitar "cannot assemble with duplicate keys"
                serie_str = serie.astype(str).str.strip()
                df["_fc"] = pd.to_datetime(serie_str, dayfirst=True, errors="coerce")
...
if perfil == "Coordinador":
    ...
    with tab1:
        ...
        if hist.empty:
            ...
            if hf.empty:
                ...
                t = len(hf)
                sc = len(hf[hf["resultado"] == "Sí contestó"])
                ...
                c3.metric("🚗 Interesados", it)
                c4.metric("❌ Rechazados", rc)
                ...
                with fa:
                    af = st.multiselect("Analista", NOMBRES_ANALISTAS, default=NOMBRES_ANALISTAS, key="af_c")
                with fr:
                    rf = st.multiselect("Resultado", RESULTADOS, default=RESULTADOS, key="rf_c")
                with fb:
                    bus = st.text_input("Buscar cédula", "", key="bus_c")
    ...
    with tab2:
        ...
        if hist.empty:
            ...
            if d.empty:
                ...
                tot = len(d)
                sr = d[d["resultado"] == "Sí contestó"]
                nr = d[d["resultado"].isin(NO_RESPONDEN)]
                g = len(sr)
                it = len(d[d["estado"] == "Interesado llega a cargue"])
                rc = len(d[d["estado"] == "Aliado Rechaza la oferta"])
                c1, c2, c3, c4, c5 = st.columns(5)
                c1.metric("📞 Total", tot)
                c2.metric("✅ Contactados", g)
                ...
                ta = (pa.merge(ga, on="analista", how="left")
                        .merge(ia, on="analista", how="left")
                        .merge(ra, on="analista", how="left")
                        .merge(na, on="analista", how="left").fillna(0))
                for c in ["gest", "inter", "rech", "noresp"]:
                    ta[c] = ta[c].astype(int)
                ...
                st.plotly_chart(
                    px.bar(ta, x="analista", y=["% gest", "% inter"],
    ...
    with tab3:
        if base is None:
            ...
            def en_pausa_fn(v):
                v = str(v).strip()
                if v in ("", "nan", "None", "NO_VOLVER", "0"):
                    return False
                f = pd.to_datetime(v, errors="coerce")
                return not pd.isna(f) and f > datetime.now()
            paus = base[base["proxima_gestion"].apply(en_pausa_fn)]
            c1, c2, c3, c4 = st.columns(4)
            c1.metric("📦 Total", len(base))
            c2.metric("✅ Disponibles", len(disp))
            c3.metric("⏸ En pausa", len(paus))
            c4.metric("🚫 Bloqueados", len(nv))
            st.markdown("---")
            disp2 = disp.copy()
            disp2["PRIORIDAD"] = disp2["dias"].apply(_prio)
    ...
    with tab4:
        ...
        st.info("Usa **Incremental** siempre que puedas para conservar el historial CRM.")
    ...
    with tab5:
        if base is None:
            ...
            if modo_a == "Asignación general (todos igual)":
                zg = st.selectbox("Zona", zonas)
                vg = st.selectbox("Vehículo", vhs)
                ...
                for a in NOMBRES_ANALISTAS:
                    ...
                    with col1:
                        z = st.selectbox("Zona", zonas, key=f"z_{a}")
                    with col2:
                        v = st.selectbox("Vehículo", vhs, key=f"v_{a}")
        ...
        if mis_ids:
            ...
            with st.form("form_g", clear_on_submit=True):
                ...
                if not fd.empty:
                    ...
                    for i, cn in enumerate(ic):
                        ci[i].metric(cn.capitalize(), str(f[cn]))
    ...
    with tab_h:
        ...
        if h_local.empty:
            ...
            if mh.empty:
                ...
                c2.metric("✅ Contactados", sc)
                c3.metric("🚗 Interesados", it)
                c4.metric("📵 No resp.", nr)
                if t > 0:
                    st.metric("% Efectividad", f"{round(it/t*100,1)}%")
                st.markdown("---")
    ...
    with tab_his:
        ...
        if h_local2.empty:
            ...
            if mhist.empty:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    fd = st.date_input("Desde", datetime.now().date() - timedelta(days=7),
                                       max_value=datetime.now().date(), key="mh_d")
                with c2:
                    fh = st.date_input("Hasta", datetime.now().date(),
                ...
                if mf.empty:
                    ...
                    t = len(mf)
                    sc = len(mf[mf["resultado"] == "Sí contestó"])
