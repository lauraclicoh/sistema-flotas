...
# ================= 3. LÓGICA DE CRUCE INCREMENTAL (del Código 1) =================
...
def procesar_subida_incremental(df_nuevo):
    ...
    - IDs existentes → actualizan datos operativos (tel, zona…) pero CONSERVAN historial CRM.
    """
    base_actual = leer_hoja("BASE")

    # Estandarizar columnas del archivo subido
    df_nuevo.columns = df_nuevo.columns.str.strip().str.lower()
    col_id = next(
        (a for a in ["identificacion", "id_aliado", "id", "cedula", "documento"]
         if a in df_nuevo.columns),
        None
    )
    if not col_id:
        st.error("No se encontró columna de identificación (Cédula/ID) en el archivo.")
        return 0, 0

    df_nuevo = df_nuevo.rename(columns={col_id: "identificacion"})
    df_nuevo["identificacion"] = df_nuevo["identificacion"].astype(str)

    # Si la base está vacía, sube todo como nuevo
    ...
    # 2. Actualizar existentes: datos operativos, conservar CRM
    ...
    base_idx.update(existentes_datos)          # sobreescribe solo las columnas que trae el nuevo archivo
...
if perfil == "Coordinador":
    ...
    # ─── CARGAR BASE (con lógica INCREMENTAL del Código 1) ───
    with tab4:
        ...
        modo_carga = st.radio(
            ...
            help=(
                ...
                "**Reemplazar**: Borra toda la base y sube el archivo nuevo."
            )
        )
        ...
        if archivo:
            try:
                ...
                if modo_carga == "🔄 Incremental (recomendado)":
                    st.markdown("""
                    **Lógica de cruce activa:**
                    - 🆕 IDs nuevos → se agregan con 0 intentos y CRM en blanco.
                    - ♻️ IDs existentes → actualizan datos operativos (teléfono, zona, etc.) **conservando** su historial de llamadas.
                    """)
                    if st.button("🚀 Ejecutar Cruce Incremental"):
                        with st.spinner("Procesando merge de bases..."):
                            n_nuevos, n_actualizados = procesar_subida_incremental(df_subido)
                        st.success(f"✅ Cruce completo: **{n_nuevos}** nuevos añadidos · **{n_actualizados}** actualizados.")
                        cargar_base.clear()
                else:
                    st.warning("⚠️ Esto **borrará toda la base actual** en Google Sheets.")
                    if st.button("♻️ Reemplazar base completa"):
                        with st.spinner("Subiendo base completa..."):
                            reemplazar_hoja("BASE", df_subido)
                            cargar_base.clear()
                        st.success(f"✅ Base reemplazada — {len(df_subido):,} aliados subidos.")
...
| No le interesa (razón) | ❌ Bloqueo permanente | Nunca |
        """)
        st.info("Estas reglas se aplican automáticamente cada vez que se guarda una gestión.")
    ...
    if "pool_activo" in st.session_state and not st.session_state["pool_activo"].empty:
        ...
        if submitted:
            ...
            if resultado == "Sí contestó" and estado_real is None:
                ...
                st.success("✅ Gestión guardada. ¡Siguiente aliado!")
                st.rerun()
